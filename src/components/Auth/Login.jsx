...
const Login = () => {
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      const response = await fetch('https://event-needs.com/backend/wp-json/jwt-auth/v1/token', {
        method: 'POST',
        headers: {
        ...
        <VStack spacing={4} align="flex-start" w="full">
          ...
          <FormControl id="username" isInvalid={errors.username} isRequired>
            ...
            <AnimatedInput
              ...
              whileFocus={{ scale: 1.1, borderColor: 'brown.500' }}
              autoFocus
            />
            {errors.username && <Text color="red.500" fontSize="sm">{errors.username}</Text>}
          </FormControl>
          <FormControl id="password" isInvalid={errors.password} isRequired>
            <FormLabel color={"brown"}>Password</FormLabel>
            <InputGroup>
              <AnimatedInput
          ...
          <Flex justifyContent="space-between" width="full" mt={4}>
            <Link color="brown.500" href="/forgot-password" fontSize="sm">
              Forgot password?
            </Link>
            <Link color="brown.500" href="/list-your-business" fontSize="sm">
              Register
            </Link>
          </Flex>
        </VStack>
